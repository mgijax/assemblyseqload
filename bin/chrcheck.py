...
results = db.sql('select _Collection_key ' + \
        'from MAP_Coord_Collection ' + \
        'where name = "%s"' % collection, 'auto')
...
results = db.sql('select a1.accID, m.symbol, cc.chromosome as geneModelChr, m.chromosome as markerChr ' + \
    ...
    'where c._Collection_key = %s ' % collKey + \
    ...
    'and a1._LogicalDB_key = %s ' % ldbKey + \
    'and a1.accID = a2.accID ' + \
    'and a2._MGIType_key = 2 ' + \
    'and a2._LogicalDB_key = %s ' % ldbKey + \
    'and a2._Object_key = m._Marker_key  ' + \
    'and cc.chromosome != m.chromosome ', 'auto')
